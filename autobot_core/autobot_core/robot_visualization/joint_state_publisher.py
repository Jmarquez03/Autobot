...
class JointStatePublisherNode(Node):
    def __init__(self):
        ...
        # This node is now primarily for visualization when not using ros2_control
        # When using ros2_control, the joint_state_broadcaster will handle this
        ...
        if not self.use_ros2_control:
            ...
            self.timer = self.create_timer(0.033, self.update_wheel_rotation)  # 30Hz updates
            
            self.get_logger().info('Joint state publisher initialized in standalone mode')
        else:
            self.get_logger().info('Joint state publisher initialized in ros2_control mode (passive)')
