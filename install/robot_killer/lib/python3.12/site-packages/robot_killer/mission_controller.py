...
class MissionController(Node):
    ...
    def deploy_hose(self, deploy=True):
        msg = String()
        if deploy:
            self.get_logger().info('Deploying fire hose')
            msg.data = "DEPLOY"
        else:
            self.get_logger().info('Retracting fire hose')
            msg.data = "RETRACT"
            
        self.hose_control_pub.publish(msg)
    ...
    def state_machine_callback(self):
        ...
        if self.state == "INIT":
            ...
            if not self.navigation_active:
                self.get_logger().info('Reached fire location, deploying hose')
                self.deploy_hose(True)
                # Wait a few seconds to simulate hose deployment
                time.sleep(3)
                self.state = "RETURNING"
                self.get_logger().info('Hose deployed, returning to start')
